...
elseif (movement(i-1,j)==2)%if the previous movement was south, next one cannot be north
    ...
    if (A(B(j,1)+1,B(j,2))~=1 && A(B(j,1)+1,B(j,2))~=-1 && A(B(j,1)+1,B(j,2))~=2 && A(B(j,1)+1,B(j,2))~=3)
        movement(i,j)=2;
    ...
    elseif (A(B(j,1)+1,B(j,2))==3 || A(B(j,1),B(j,2)+1)==3 || A(B(j,1),B(j,2)-1)==3)
        [A,B,movement,i,j]=crossing(A,B,i,j,movement,3);
        %south
...
elseif(movement(i-1,j)==8)%if the previous movement was north, next one cannot be south
    %control for another car
    if (A(B(j,1)-1,B(j,2))~=1 && A(B(j,1)-1,B(j,2))~=-1 && A(B(j,1)-1,B(j,2))~=3)
        movement(i,j)=8;
    elseif (A(B(j,1)-1,B(j,2))==3 || A(B(j,1),B(j,2)+1)==3 || A(B(j,1),B(j,2)-1)==3)
        [A,B,movement,i,j]=crossing(A,B,i,j,movement,3);
        %north
...
elseif(movement(i-1,j)==6)%if the previous movement was east, next one cannot be west
    %control for another car
    if (A(B(j,1),B(j,2)+1)~=1 && A(B(j,1),B(j,2)+1)~=-1 && A(B(j,1),B(j,2)+1)~=3)
        movement(i,j)=6;
    elseif (A(B(j,1)+1,B(j,2))==3 || A(B(j,1)-1,B(j,2))==3 || A(B(j,1),B(j,2)+1)==3)
        [A,B,movement,i,j]=crossing(A,B,i,j,movement,3);
        %east
    elseif (A(B(j,1),B(j,2)+1)==1)
        [A,B,movement]=move(A,B,i,j,movement,6);
